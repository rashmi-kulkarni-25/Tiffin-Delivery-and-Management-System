...
function EditVendorProfile() {
    ...
    const getMyProfile = ()=>
    {
      ...
      const url = createaUrl('vendor/getvendorbyid')
    ...
    {
        debugger
      var obj = {"vendor_id": vendorId, "name":name, "address":address, "pincode":pincode,
    "email":email, "mob_no":mobNo};
      var helper = new XMLHttpRequest();
      helper.onreadystatechange = ()=>{
        debugger
        if (helper.readyState === 4 && helper.status === 200 )
      ...
      const url = createaUrl('vendor/updateprofile')
      helper.open("PUT", url);
      helper.setRequestHeader("Content-Type", "application/json");
...
<label><h4>Mobile Number</h4></label>
          ...
          <center>
            <button className="btn btn-success btn-lg my-3" onClick={update}>Update</button>
            </center>
                </div>
                <div className="col-md-3"></div>
            </div>
