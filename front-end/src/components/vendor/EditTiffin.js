...
function EditTiffin() {
    ...
    const getMyTiffin = ()=>
    {
      ...
      const url = createaUrl('vendor/getmytiffin')
    ...
    const handleCategoryChange = (event) => {
      ...
      {
        debugger;
        var tiffin = {"tiffin_id": tiffinId, "tiffin_name": tiffinName, "description": description,
    "tiffin_category": category, "tiffin_price": price, "vendor_id": vendorId,
"image_link": image, "status": status}
        var helper = new XMLHttpRequest();
        helper.onreadystatechange = ()=>{
        debugger
        if (helper.readyState === 4 && helper.status === 200 )
        ...
        const url = createaUrl('vendor/updatetiffin')
        helper.open("PUT", url);
        helper.setRequestHeader("Content-Type", "application/json");
...
<center>
    ...
    <input clasName="form-check-input" type="radio" value="veg" name="category" id="flexRadioDefault1"
    checked={category === "veg"} onChange={handleCategoryChange}/>
    <label clasName="form-check-label mx-3" for="flexRadioDefault1">
        <h4 className="mx-3">Veg</h4>
    </label>

    <input clasName="form-check-input" type="radio" value="nonveg" name="category" id="flexRadioDefault2"
    checked={category === "nonveg"} onChange={handleCategoryChange}/>
    <label clasName="form-check-label mx-3" for="flexRadioDefault2">
        <h4 className="mx-3">Non-veg</h4>
          ...
          <center>
            <button className="btn btn-primary btn-lg my-3" onClick={updatetififn}>Update</button>
            </center>
                </div>
                <div className="col-md-3"></div>
            </div>
